...
def eliminarInalcancaveis(afnd):
    visitados = set()
    def elimina(regra, estado):        # faz busca em profundidade e remove os inalcançáveis
    ...
    x = len(afnd)
    for i in range(x):
        if i not in visitados:      # elimina os estados não visitados
            del afnd[i]
...
def eliminarInuteis(afnd):
    ...
    for rg in afnd:
        if '*' in afnd[rg].keys():
            uteis.add(rg)
    def elimina(regra, nRegra):     # faz busca em profundidade e encontra os inúteis
        if nRegra in uteis:
            return True
